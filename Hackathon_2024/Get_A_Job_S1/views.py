...
def create_job(request):
    ...
    return render(request, 'Get_A_Job_S1/create_job.html',{'form': form})
...
def postuler_job(request,job_id):
    ...
    return render(request, 'Get_A_Job_S1/job_post.html',{'form': form,'posts':posts,'offre': offre})

def Action_admin(request):
    return render(request, 'Get_A_Job_S1/admin.html')

@user_passes_test(est_administrateur)
@login_required
def select_offre(request,objet):
    offres =JobOffer.objects.all()
    return render(request, 'Get_A_Job_S1/select_offre.html', {'offres':offres, 'objet':objet})
